...
func (s *websocketServiceTestSuite) TestPartialDepthServe() {
	...
	doneC, stopC, err := WsPartialDepthServe("ETHBTC", "5", MainNet, func(event *WsPartialDepthEvent) {
...
func (s *websocketServiceTestSuite) TestPartialDepthServe100Ms() {
	...
	doneC, stopC, err := WsPartialDepthServe100Ms("ETHBTC", "5", MainNet, func(event *WsPartialDepthEvent) {
		e := &WsPartialDepthEvent{
			Symbol:       "ETHBTC",
			LastUpdateID: 160,
			Bids: []Bid{
...
func (s *websocketServiceTestSuite) TestCombinedPartialDepthServe() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, err := WsCombinedPartialDepthServe(symbolLevels, MainNet, func(event *WsPartialDepthEvent) {
		e := &WsPartialDepthEvent{
			Symbol:       "ETHUSDT",
			LastUpdateID: 160,
			Bids: []Bid{
...
func (s *websocketServiceTestSuite) TestDepthServe() {
	...
	doneC, stopC, err := WsDepthServe("ETHBTC", MainNet, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Event:         "depthUpdate",
			Time:          1499404630606,
			Symbol:        "ETHBTC",
...
func (s *websocketServiceTestSuite) TestDepthServe100Ms() {
	...
	doneC, stopC, err := WsDepthServe100Ms("ETHBTC", MainNet, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Event:         "depthUpdate",
			Time:          1499404630606,
			Symbol:        "ETHBTC",
...
func (s *websocketServiceTestSuite) TestCombinedDepthServe() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, err := WsCombinedDepthServe(symbols, MainNet, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Symbol:        "BTCUSDT",
			Time:          1629769560797,
...
func (s *websocketServiceTestSuite) TestCombinedDepthServe100Ms() {
	...
	s.mockWsServe(data, errors.New(fakeErrMsg))
	defer s.assertWsServe()
	doneC, stopC, err := WsCombinedDepthServe100Ms(symbols, MainNet, func(event *WsDepthEvent) {
		e := &WsDepthEvent{
			Symbol:        "BTCUSDT",
			Time:          1629769560797,
			LastUpdateID:  13544037,
...
func (s *websocketServiceTestSuite) TestKlineServe() {
	...
	doneC, stopC, err := WsKlineServe("ETHBTC", "1m", MainNet, func(event *WsKlineEvent) {
		e := &WsKlineEvent{
			Event:  "kline",
			Time:   1499404907056,
			Symbol: "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsAggTradeServe() {
	...
	doneC, stopC, err := WsAggTradeServe("ETHBTC", MainNet, func(event *WsAggTradeEvent) {
		e := &WsAggTradeEvent{
			Event:                 "aggTrade",
			Time:                  1499405254326,
			Symbol:                "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedKlineServe() {
	...
	doneC, stopC, err := WsCombinedKlineServe(input, MainNet, func(event *WsKlineEvent) {
		e := &WsKlineEvent{
			Event:  "kline",
			Time:   1499404907056,
			Symbol: "ETHBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedAggTradeServe() {
	...
	doneC, stopC, err := WsCombinedAggTradeServe([]string{"ETHBTC"}, MainNet, func(event *WsAggTradeEvent) {
		e := &WsAggTradeEvent{
			Event:                 "aggTrade",
			Time:                  1499405254326,
			Symbol:                "ETHBTC",
...
func (s *websocketServiceTestSuite) testWsUserDataServe(data []byte, expectedEvent *WsUserDataEvent) {
	...
	doneC, stopC, err := WsUserDataServe("fakeListenKey", MainNet, func(event *WsUserDataEvent) {
...
func (s *websocketServiceTestSuite) TestWsMarketStatServe() {
	...
	doneC, stopC, err := WsMarketStatServe("BNBBTC", MainNet, func(event *WsMarketStatEvent) {
		e := &WsMarketStatEvent{
			Event:              "24hrTicker",
			Time:               123456789,
			Symbol:             "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedMarketStatServe() {
	...
	doneC, stopC, err := WsCombinedMarketStatServe([]string{"BNBBTC"}, MainNet, func(event *WsMarketStatEvent) {
		e := &WsMarketStatEvent{
			Event:              "24hrTicker",
			Time:               123456789,
			Symbol:             "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsAllMarketsStatServe() {
	...
	doneC, stopC, err := WsAllMarketsStatServe(MainNet, func(event WsAllMarketsStatEvent) {
...
func (s *websocketServiceTestSuite) TestWsTradeServe() {
	...
	doneC, stopC, err := WsTradeServe("BNBBTC", MainNet, func(event *WsTradeEvent) {
		e := &WsTradeEvent{
			Event:         "trade",
			Time:          123456789,
			Symbol:        "BNBBTC",
...
func (s *websocketServiceTestSuite) TestWsCombinedTradeServe() {
	...
	doneC, stopC, err := WsCombinedTradeServe([]string{"BNBBTC"}, MainNet, func(event *WsCombinedTradeEvent) {
...
func (s *websocketServiceTestSuite) TestWsAllMiniMarketsStatServe() {
	...
	doneC, stopC, err := WsAllMiniMarketsStatServe(MainNet, func(event WsAllMiniMarketsStatEvent) {
...
func (s *websocketServiceTestSuite) TestBookTickerServe() {
	...
	doneC, stopC, err := WsBookTickerServe("BTCUSD_200626", MainNet, func(event *WsBookTickerEvent) {
		e := &WsBookTickerEvent{
			UpdateID:     17242169,
...
func (s *websocketServiceTestSuite) TestAllBookTickerServe() {
	...
	doneC, stopC, err := WsAllBookTickerServe(MainNet, func(event *WsBookTickerEvent) {
		e := &WsBookTickerEvent{
			UpdateID:     17242169,
			Symbol:       "BTCUSD_200626",
			BestBidPrice: "9548.1",
